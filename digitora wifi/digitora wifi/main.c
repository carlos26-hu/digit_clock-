...
void UART1AdatKuld(char data);
void oramu(void);
void orakijelzes(void);
void orabeallitas (void);
void stopper(void);
void SendStringOnUart1 (char *ip);

/********************* DECLARATION OF GLOBAL VARIABLES *************************/
volatile int Counter = 0,ora=0,perc=0,mp=0,szmp=0, ertek=0,beall=0;
volatile int  sora =0,sperc =0,smp=0,smp1=0,szmp1=0,stop=0; 
volatile char UtoljaraLenyomottGomb='0', temp='N', felenged = 1,i=0,Receivedbyte1=0;
...
int main(void)
{
	...
	sei();								//glob�lis megszakit�sok enged�lyez�se
	orakijelzes();
	orabeallitas();
	while (1)
	{
		...
		{	
			case 1:beall=1;break;
			case 2:beall=2;break;	//stopper inditas
			case 4:beall=3;break;	//stopper k�rido
			case 8:beall=4;break;	//stopper reset
			case 16:beall=5;break;
			default:break;	
		}
...
// UART1 9600 bps soros kommunikacio sebesseg beallitasa
...
void UART1AdatKuld(char data) // Ez a fuggveny a kuldendo adatot beirja az UDR regiszter kimeno pufferjebe
{
...
void stopper(void)
{
	...
	if(szmp==100)		// ha szmp eleri a 100-at
	{
		...
		szmp=0;			//szmp-t null�zzuk
	}
	if (smp==60)
	{
		sperc++;		//percet noveljuk
		smp=0;			//mp-t null�zzuk
	}
...
{	
		...
		if (((Counter%6)==0) && (beall ==2 || beall ==3)) //ha szamlalo marad�k n�lk�l oszthat� 6al, �s el van inditva a stopper 
		{	
			stopper();									//stopper l�ptet�s 0.1 mpenk�nt
		}
...
ISR(USART1_RX_vect)  
{
	UtoljaraLenyomottGomb = UDR1; // Fetch the received byte value into the variable "ByteReceived"
	UART1AdatKuld(UtoljaraLenyomottGomb);
